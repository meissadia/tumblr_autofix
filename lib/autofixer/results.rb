module DK
  class Idable
    ...
    def show_results
      ...
      if @clear_r
        @no_info.each do |post|
          success = update_post_comment(post, @prefix)
          @updated << ['unknown', @prefix, link_to_edit(post)] if success
        end
        @no_info = []
      end
      ...
      unless @no_info.empty?
        ...
        puts "** #{size} have insufficient info for automated processing **:\n"
    ...
    def generate_review_webpage(post_info, fname)
      ...
      post_info.each_slice(4) do |posts|
        pg += '<tr>'
        posts.each{|post| pg += post_image_code(DK::Post.new(post))}
        pg += '</tr>'
      end
      pg += '</table></body></html>'
    ...
    def post_image_code(post)
      ...
      res += "<p>#{photo.caption}</p><br><p>#{post.tags}</p></td>"
    end
